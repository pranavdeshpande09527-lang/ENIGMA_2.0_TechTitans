        ...
        <div className="glass-card overflow-hidden transition-all duration-300">
            <div className="px-6 py-4 border-b border-white/5 flex justify-between items-center bg-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-white">Health Profile</h3>
                        <div className="flex gap-2 mt-1">
                            ...
                            {!hasHighVulnerabilty && !profile.smoker && (
                                <span className="text-[10px] bg-safe-500/20 text-safe-400 px-2 flex items-center rounded-sm font-semibold border border-safe-500/30">
                                    Standard Profile
                                </span>
                            )}
                        </div>
            ...
            {isEditing && (
                <div className="p-6 space-y-4 animate-slide-up bg-black/20">
                    <div className="grid grid-cols-2 gap-4">
                        <label className="text-sm text-gray-400 flex flex-col gap-1.5">
                            Age
                            <input type="number" name="age" value={profile.age} onChange={handleChange} className="bg-white/5 border border-white/10 rounded-lg p-2 text-white text-sm" />
                        </label>
                        <label className="text-sm text-gray-400 flex flex-col gap-1.5">
                            Outdoor Hours / Day
                            <input type="number" name="outdoorHours" value={profile.outdoorHours} onChange={handleChange} className="bg-white/5 border border-white/10 rounded-lg p-2 text-white text-sm" />
                        </label>
                    </div>
                    ...
                    <div className="pt-2">
                        <button onClick={handleSave} className="w-full bg-primary-600 hover:bg-primary-500 text-white font-medium py-2 rounded-lg transition-colors">
                            Save Profile
                        </button>
                    </div>
                </div>
